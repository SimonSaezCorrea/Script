-- Definición de variables de fecha (Rango de ejecución)
...
-- SEGUNDA CONSULTA: AGRUPACIÓN POR MODELO DE NEGOCIO (TYPE)
SELECT 
    "companyType",
    COUNT(*) as "totalCompanies", -- Cuantas empresas hay por tipo
    SUM("totalUsersAllTime") as "totalUsersAllTime",
    SUM("activeUsers") as "activeUsers",
    SUM("membershipsPurchased") as "membershipsPurchased",
    SUM("activeMemberships") as "activeMemberships",
    SUM("totalMembershipsHistory") as "totalMembershipsHistory",
    SUM("totalPremiumUsers") as "totalPremiumUsers",
    SUM("totalEliteUsers") as "totalEliteUsers",
    SUM("newUsersTotal") as "newUsersTotal",
    SUM("newPremiumUsers") as "newPremiumUsers",
    SUM("newEliteUsers") as "newEliteUsers",
    SUM("protectedPetsCount") as "protectedPetsCount"
FROM summary
GROUP BY "companyType"
ORDER BY "companyType" ASC;