"""
Script para comparar datos entre archivo de altas Mercer y archivo BICE Southbridge.

Compara RUTs y reporta inconsistencias.

Archivos esperados en carpeta `data/Southbridge`:
- Altas SOAP al31012026-MERCER.xlsx (CARGA - Altas a agregar)
...
def normalizar_ruts_dataframe_mercer(df, col_rut, col_dv=None):
    """
...
def comparar_mercer():
    """
    Compara los RUTs entre el archivo de altas Mercer y el archivo BICE de Southbridge.
    """
    # Rutas de archivos
    ...
    for filename in os.listdir(data_dir):
        ...
        if 'Altas SOAP' in filename and 'MERCER' in filename:
    ...
    if not archivo_carga or not archivo_bice:
        ...
        print(f"   Carga Mercer encontrado: {archivo_carga is not None}")
        print(f"   BICE Southbridge encontrado: {archivo_bice is not None}")
        return
    
    print("="*80)
    print("📊 COMPARACIÓN ALTAS vs BICE - MERCER (SOUTHBRIDGE)")
    print("="*80)
    ...
    df_carga = normalizar_ruts_dataframe_mercer(df_carga, 'Rut propietario', 'Propietario DV')
    
    # Normalizar RUTs en archivo BICE (RUT completo)
    df_bice = normalizar_ruts_dataframe_mercer(df_bice, 'RUT')
    ...
    df_coincidencias, df_inconsistencias, archivos = separar_y_guardar_resultados(
        df_resultados, script_dir, timestamp, orden_estados, prefijo='mercer_'
    )
    ...
    if len(df_carga_sin_bice) > 0:
        ...
        archivo_csv_carga = os.path.join(resultado_dir, f'carga_sin_bice_mercer_{timestamp}.csv')
    ...
    if len(df_bice_sin_carga) > 0:
        ...
        # Guardar en formato especial
        from utils.file_handlers import guardar_csv_formato_especial
        archivo_csv_especial = os.path.join(resultado_dir, f'bice_sin_carga_mercer_{timestamp}.csv')
