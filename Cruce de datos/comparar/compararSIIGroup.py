...
def comparar_sii_group():
    ...
    if len(df_carga_sin_bice) > 0:
        ...
        df_csv_carga = pd.DataFrame({
            ...
            'Apellido': '',  # SII Group no separa apellidos
        ...
        archivo_csv_carga = os.path.join(resultado_dir, f'carga_sin_bice_sii_group_{timestamp}.csv')
    ...
    if len(df_bice_sin_carga) > 0:
        # Preparar DataFrame con el formato requerido (SII Group solo tiene Nombre completo)
        ...
        # Guardar en formato especial
        from utils.file_handlers import guardar_csv_formato_especial
        archivo_csv_especial = os.path.join(resultado_dir, f'bice_sin_carga_sii_group_{timestamp}.csv')
